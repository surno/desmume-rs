...
use std::ops::{Deref, DerefMut, Range, RangeFrom, RangeFull, RangeInclusive, RangeTo, RangeToInclusive};
...
pub struct TypedMemoryReader<'a, T: MemType>(pub(crate) &'a DeSmuMEMemory, pub(crate) PhantomData<T>);
...
/// using `u32`'s or ranges over `u32`s that address a specific space in the NDS memory. The value
/// returned is the data at those memory locations in the format specified by `T` (eg. `u8`, `i16`, `u32`, etc.).
pub struct TypedMemoryWriter<'a, T: MemType>(pub(crate) & 'a mut DeSmuMEMemory, pub(crate) PhantomData<T>);
...
impl_write_access!(TypedMemoryWriter, u8, u8, desmume_memory_read_byte, desmume_memory_write_byte);
...
impl_write_access!(TypedMemoryWriter, u16, u16, desmume_memory_read_short, desmume_memory_write_short);
...
impl_write_access!(TypedMemoryWriter, u32, c_ulong, desmume_memory_read_long, desmume_memory_write_long);
...
impl_write_access!(TypedMemoryWriter, i8, u8, desmume_memory_read_byte_signed, desmume_memory_write_byte);
...
impl_write_access!(TypedMemoryWriter, i16, u16, desmume_memory_read_short_signed, desmume_memory_write_short);
...
impl_write_access!(TypedMemoryWriter, i32, c_ulong, desmume_memory_read_long_signed, desmume_memory_write_long);
...
impl<'a, T, U, const W: bool> Deref for MemIndexWrapper<T, U, W> { type Target = T; fn deref(&self) -> &Self::Target { &self.0 } }
impl<'a, T, U, const W: bool> DerefMut for MemIndexWrapper<T, U, W> { fn deref_mut(&mut self) -> &mut Self::Target { &mut self.0 } }

impl<T: MemType, U, const W: bool> IndexMove<u32> for MemIndexWrapper<U, T, {W}> where U: MemoryAccess<T, W> {
    type Output = T;
...
impl<T: MemType, U, const W: bool> IndexMove<Range<u32>> for MemIndexWrapper<U, T, {W}> where U: MemoryAccess<T, W> {
    type Output = Vec<T>;
...
impl<T: MemType, U, const W: bool> IndexMove<RangeFrom<u32>> for MemIndexWrapper<U, T, {W}> where U: MemoryAccess<T, W> {
    type Output = Vec<T>;
...
impl<T: MemType, U, const W: bool> IndexMove<RangeFull> for MemIndexWrapper<U, T, {W}> where U: MemoryAccess<T, W> {
    type Output = Vec<T>;
...
impl<T: MemType, U, const W: bool> IndexMove<RangeInclusive<u32>> for MemIndexWrapper<U, T, {W}> where U: MemoryAccess<T, W> {
    type Output = Vec<T>;
...
impl<T: MemType, U, const W: bool> IndexMove<RangeTo<u32>> for MemIndexWrapper<U, T, {W}> where U: MemoryAccess<T, W> {
    type Output = Vec<T>;
...
impl<T: MemType, U, const W: bool> IndexMove<RangeToInclusive<u32>> for MemIndexWrapper<U, T, {W}> where U: MemoryAccess<T, W> {
    type Output = Vec<T>;
...
impl<T: MemType, U> IndexSet<u32> for MemIndexWrapper<U, T, true> where U: MemoryAccess<T, true> {
    fn index_set(&mut self, index: u32, value: &Self::Output) {
        unsafe { self.write(index, *value) }
    }
}

impl<T: MemType, U> IndexSet<Range<u32>> for MemIndexWrapper<U, T, true> where U: MemoryAccess<T, true> {
    fn index_set(&mut self, index: Range<u32>, value: &Self::Output) {
        unsafe { self.write_range(index.start, index.end - 1, value) }
    }
}

impl<T: MemType, U> IndexSet<RangeFrom<u32>> for MemIndexWrapper<U, T, true> where U: MemoryAccess<T, true> {
    fn index_set(&mut self, index: RangeFrom<u32>, value: &Self::Output) {
        unsafe { self.write_range(index.start, END_OF_MEMORY, value) }
    }
}

impl<T: MemType, U> IndexSet<RangeFull> for MemIndexWrapper<U, T, true> where U: MemoryAccess<T, true> {
    fn index_set(&mut self, _index: RangeFull, value: &Self::Output) {
        unsafe { self.write_range(START_OF_MEMORY, END_OF_MEMORY, value) }
    }
}

impl<T: MemType, U> IndexSet<RangeInclusive<u32>> for MemIndexWrapper<U, T, true> where U: MemoryAccess<T, true> {
    fn index_set(&mut self, index: RangeInclusive<u32>, value: &Self::Output) {
        unsafe { self.write_range(*index.start(), *index.end(), value) }
    }
}

impl<T: MemType, U> IndexSet<RangeTo<u32>> for MemIndexWrapper<U, T, true> where U: MemoryAccess<T, true> {
    fn index_set(&mut self, index: RangeTo<u32>, value: &Self::Output) {
        unsafe { self.write_range(START_OF_MEMORY, index.end - 1, value) }
    }
}

impl<T: MemType, U> IndexSet<RangeToInclusive<u32>> for MemIndexWrapper<U, T, true> where U: MemoryAccess<T, true> {
